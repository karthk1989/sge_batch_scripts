...
# This file:  sge_na61_loop.sh
# Loops through number of flux throws
# For each flux throw submits an instance of sge_na61_job.sh
# e.g.
#      ./sge_na61_loop.sh 500
...
for i in $(seq 0 $N)
do 
  echo qsub -q sge_snextbar_job.sh $i  >>$LOG
  qsub -l h_rss=2G,h_vmem=2G  -q SL6 sge_snextbar_job.sh $i  |tee -a $LOG
done
